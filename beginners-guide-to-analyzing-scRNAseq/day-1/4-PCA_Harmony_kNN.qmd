...
Just by visualizing these two matrices, we can start to understand the major axes of variance of our data. Seurat has built-in functions to graph the PCs and loadings
```{r}
...
In normal data science, typically we include PCs until we have reached some threshold of variance explained, often 85%. But if we calculate the total variance explained by each PC up to the cutoff we chose by ElbowPlot, we can see it's only 20%. This is because scRNAseq data are highly noisy with a great deal of unique variance per cell. We would have to include 100s of PCs to reach 85% variance explained.
...
plot_batch_effect_heatmap <- function(seurat_object, 
                                     latent_space = 'pca', 
                                     num_pcs = 20, 
    ...
    metadata[categorical_cols] <- lapply(metadata[categorical_cols], as.factor) # convert character columns to factors for one-hot encoding
...
plot_batch_effect_heatmap(se, 
                        metadata_columns_to_include = c('Age', 
                                                        'sex', 
                                                        "donor_id", 
                                                        "Number of UMI", 
                                                        "disease",
