...
saveRDS(
    object = se@assays$RNA$counts,
    file = "../data/counts.rds")
```

#### Reference Mapping

We download the reference data from [Zenodo](https://zenodo.org/records/4546839)
```{r}
# system("curl -O https://zenodo.org/records/4546839/files/ref.Rds")
...
```{r fig.width=12, fig.height=5}
DimPlot(se, group.by = c("annotation_lvl1", "predicted.id"))
```

We can also check the overlap as follows:
```{r}
se@meta.data %>%
