...
p3 <- ggplot(se@meta.data, aes(x = pANN)) +
    geom_density(color = "#6abcb6", fill = "#6abcb6", alpha = 0.7) +
    # scale_x_continuous(
    #     # transform = "log10",
    #     labels = scales::unit_format(unit = "K", scale = 1)) +
    theme_classic()
p3
```
...
Filter out our data by 4 criteria. 1. Any cell with less that 500 UMIs 2. Any cell with less than 200 genes 3. Any cell with more than 15% mitochondrial genes 4. Any cell with a pANN value greater than 0.25

```{r, fig.height=6, fig.width=12}
se <- subset(
  se, 
  subset = nFeature_RNA > 200 & nCount_RNA > 500 & perc.mt < 15 & pANN < 0.25)
