...
```{r}
# getwd() 
# setwd('./workshops/navigating-the-single-cell-landscape-a-comprehensive-workshop-on-scrnaseq/')
```
...
The AggregateExpression() function arranges the cell by gene matrix to group gene counts by the Sample ID and labelled Celltype of each cell. It outputs either a new seurat object or a list of counts. **Aggregation is the pseudobulking step where genes are summed across cells per celltype per sample.**
...
annot_pseudobulk <- AggregateExpression(
  ...
  )$RNA # desired assay defined here
# NOTE: Can add "return.seurat = FALSE" and assign this to a variable which will output as a list of counts
...
3.  design = Tells DESeq what comparisons to make.
...
saveRDS(object = res, 
        file = './day-1/pseudobulk_analysis/results/cd4_annot_pseudobulk_results.rds')
...
results notes: - positive log2fold change are upregulated, and negative are down reegulated - can set the p-value to a certain threshold
...
lapply(unique(colData$CT), function(cluster) {
  ...
  saveRDS(
    res,
    glue::glue("./day-1/pseudobulk_analysis/results/{r}_fluVnorm_annot_pseudobulk_results.rds"))
  ...
  write_csv(
    restbl,
    glue::glue("./day-1/pseudobulk_analysis/results/{r}_fluVnorm_annot_pseudobulk_results.csv"))
})
...
```{r}
# res <- read_csv('./day-1/pseudobulk_analysis/results/cd4_annotation_pseudobulk_results.csv')
# 
# res %>% head

res <- readRDS('./day-1/pseudobulk_analysis/results/cd4_annotation_pseudobulk_results.rds')
```

TD: Save. all data so if a collaborator asks what the comparison was, you have the header stored or rerun analysis.
...
```{r}
res_ls <- list.files('./day-1/pseudobulk_analysis/results/', pattern = "*.rds")
```
...
lapply(res_ls, function(file) {
    res_df <- readRDS(glue::glue('./day-1/pseudobulk_analysis/results/{file}'))
    pdf(glue::glue('./day-1/pseudobulk_analysis/figures/',strsplit(file,split='_')[[1]][1],'_volcano.pdf'),width=14,height=10)
...
```{r}
list.files('./day-1/pseudobulk_analysis/figures/')
```
...
annots <- list.files('./day-1/pseudobulk_analysis/results')[grepl('annotation_',list.files('./day-1/pseudobulk_analysis/results'))]
clusts <- list.files('./day-1/pseudobulk_analysis/results')[grepl('cluster-',list.files('./day-1/pseudobulk_analysis/results'))]

annots = annots[c(-1,-8)]
```

```{r}
setwd('./day-1/pseudobulk_analysis/results')
