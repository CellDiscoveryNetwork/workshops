...
```{r}
se <- readRDS("./day-1/data/workshop-data.rds")
```
...
se@meta.data['cell_type']
```

```{r}
# Correct for function formatting preferences

genes <- rownames(se) %>% str_replace_all('-','.')
genes[0:10]
```
...
```{r}
srobj <- CreateSeuratObject(data,meta.data = se@meta.data)

srobj
```

```{r}
row.names(srobj[["RNA"]]$counts) %>% head
```

```{r}
AggregateExpression(srobj, group.by = c('Sample ID','Celltype'))$RNA %>% head
