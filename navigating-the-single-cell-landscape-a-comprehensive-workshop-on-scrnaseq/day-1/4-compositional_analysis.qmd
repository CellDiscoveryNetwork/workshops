...
celltypePercentagesDF %>% 
    ggplot(aes(x = `Sample ID`, y = pct_celltype, fill = Celltype), color = 'white') + 
...
plots = plot(sccomp_res)

plots$boxplot + ggtitle("Box plots showing sccomp transforms")

```

In sccomp, a differential composition "c" and differential variability "v" are both calculated between groups. Here Flu vs. Normal.
...
EnhancedVolcano(sccomp_res, x = "c_effect", y = "c_FDR", lab = sccomp_res$Celltype, pCutoff = 0.05, title = "sccomp differential composition Flu vs. Normal", subtitle = "p-val is FDR. FDR cutoff = 0.05; Flu positive, Normal negative")
```
