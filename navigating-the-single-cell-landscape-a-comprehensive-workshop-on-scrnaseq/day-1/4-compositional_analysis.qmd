...
When there are many cell states in a dataset, for example in a large dataset, it can be nice to make a volcano plot of results, so here we include code for creating a nice looking One
```{r}
EnhancedVolcano(wilcoxon_comparisons %>% filter(transform=='Percent'), x = stati)
```
